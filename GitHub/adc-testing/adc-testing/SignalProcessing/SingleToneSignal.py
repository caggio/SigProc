...
class SingleToneSignal(Signal):
    ...
    def precalculateAll(self):
        ...
        if self.fullnsamples > 0:
            ...
            # fit the sine 
            self.w0, self.A, self.B, self.C  = Sinefit.sinefit4(data, 1.0/rate, w0, 1e-12, 200)
        ...
        self.realHistogram = self._histogram() if (not skip) and self.nsamples > 0 else np.array([0])
        self.idealHistogram = self._ideal_histogram() if (not skip) and self.nsamples > 0 else np.array([0])
        ...
        if skip:
            ...
            print('calculating DNL INL',self.maxDNL,self.maxINL)
    ...
    def precalculate(self, windowName):
        ...
        output = WindowedSignal.WindowedSignal()
        ...
        output.dft = fft.rfft(output.data)
        ...
        output.ldft = 20*log10(output.dft)
        output.ludft = 20*log10(output.udft)
        ...
        thindex = np.vstack(list(map(lambda x: x[0], tenHarmonics)))
        thvalues = np.vstack(list(map(lambda x: x[2], tenHarmonics)))

        tenHarmonicsValues = np.array(list(map(lambda x: x[2], tenHarmonics)))
        ...
        noiseMask = np.array(ones(len(output.dft)))
        noiseMask[0] = 0
